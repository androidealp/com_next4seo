...
$pagination = $displayData['data']['pagination'];
        ...
        <tr id="rowtable_<?=$column['id']?>">
          ...
          <td>
            ...
            <?php
              ...
              $slug = $column['id']. ':' . $column['alias'];
              $link_pure = ContentHelperRoute::getArticleRoute($slug, $column['catid'], $column['language']);

              $siteApp = JApplication::getInstance('site');
              $siteRouter = $siteApp->getRouter();
              $siteMenu = $siteApp->getMenu();
              $root = JURI::root();
              $link = str_replace('/administrator',NULL,$siteRouter->build($link_pure));

             ?>
            <a target="_blank" style="display:block; width:100%" href="<?=$link?>"><?=$column['title']?></a>
          </td>
          <td>
 ...
 <div id="alertas" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
   <div class="modal-header">
     ...
     <h3 id="myModalLabel">Salvar Artigo</h3>
   </div>
...
<div class="row-fluid show-grid">
 ...
 <script type="text/javascript">
   jQuery(function($){
     ...
     $('.row-fluid #toolbar').html('<a href="index.php?option=com_next4seo&view=csv&action=joomla" class="btn btn-small btn-success"><i class="fa fa-save"></fa> Exportar em CSV </a>');
     ...
     $('[data-nextsubmit]').on('click',function(e){
       ...
       $.ajax({
         url:'index.php?option=com_next4seo&view=joomla&action=ajaxsend',
         method:'POST',
         data:datafields,
         dataType:'JSON',
         beforeSend:function()
